...
bool is_keyword(const char *str)
{
  return
    // Types (default)
    S_EQ(str, "unsigned") || S_EQ(str, "signed") || S_EQ(str, "char") ||
    ...
    S_EQ(str, "const") || S_EQ(str, "enum") ||

    // Cancer (maybe)
    S_EQ(str, "i8") || S_EQ(str, "i8") || S_EQ(str, "i16") ||
    S_EQ(str, "i32") || S_EQ(str, "i64") || S_EQ(str, "i128") ||
    S_EQ(str, "isize") || S_EQ(str, "u8") || S_EQ(str, "u16") ||
    S_EQ(str, "u32") || S_EQ(str, "u64") || S_EQ(str, "u128") ||
    S_EQ(str, "usize") ||
    ...
    S_EQ(str, "goto") || S_EQ(str, "typedef") || S_EQ(str, "extern") ||
    S_EQ(str, "restrict");
}
...
struct token *token_make_multi_line_comment()
{
  ...
  while (1) {
    ...
    if (c == EOF) {
      compiler_error(lex_process->compiler,
                     "You did not close this multi line comment\n");
    }
    else if (c == '*') {
      // TODO: Skip the *
      nextc();
...
static struct token *token_make_identifier_or_keyword()
{
  ...
  if (is_keyword(buffer_ptr(buffer))) {
    return token_create(
      &(struct token){.type = TOKEN_TYPE_KEYWORD, .sval = buffer_ptr(buffer)});
  }
